---
title: DLL and Code Injection
date: 2017-02-28 11:54:09
tags:
- windows
- injection
categories:
...
很久没有研究Windows环境下的exploit了，最近没事看了python greyhat那本书，特地把注入方面的看了一下，记录一下。

## DLL Injection

这种方法利用的是CreateRemoteThread()函数，利用过程挺简单的。
...
```
需要注意的是lpStartAddress和lpParameter这两个参数。lpStartAddress是线程函数的起始地址，lpParameter为传给该函数的参数。
<!-- more -->

```python
'''
dll_injector.py <pid to injector> <dll_path>
'''
import sys
from ctypes import *
...
#get the remote process's handle
h_process = kernel32.OpenProcess(PROCESS_ALL_ACCESS, False, int(pid))

if not h_process:
	print "[*] Could't acquire a handle to PID: %s" % pid
	sys.exit(0)

#allocate memory for the strig of DLL
...
## Code Injection
Code Injection跟上面的DLL Injection利用思路差不多
